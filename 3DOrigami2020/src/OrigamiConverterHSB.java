...
public class OrigamiConverterHSB {
    ...
    // list of colors and their HSB values
    private HashMap<Integer, float[]> HSBList = new HashMap<Integer, float[]>();
    ...
    // gives each RSB value in the colorList a HSB value
    public void setColors(){
        for (Map.Entry<Integer, String> entry : colorList.entrySet()) {
            // convert to hsb and put it in hasmap of color integer and float
            int red = (entry.getKey() >> 16) & 0xFF;
            int green = (entry.getKey() >> 8) & 0xFF;
            int blue = entry.getKey() & 0xFF;

            float[] hsb = Color.RGBtoHSB(red, green, blue, null);
            HSBList.put( entry.getKey(), hsb );
        }
    }
    public void printHSB(){
        for (Map.Entry<Integer, float[]> entry : HSBList.entrySet()) {
    ...
    public int getClosest(int color){
        if( colorDex.containsKey( color ) ){
            ...
            // change to RGB then convert to HSB
            ...
            for (Map.Entry<Integer, float[]> entry : HSBList.entrySet()) {
                float newDistance = (float)Math.sqrt(  Math.pow( entry.getValue()[0] - hsb[0], 2) + 
                                                Math.pow( entry.getValue()[1] - hsb[1], 2) + 
                                                Math.pow( entry.getValue()[2] - hsb[2], 2) );
    ...
    public static void main(String[]args){
        ...
        OrigamiConverterHSB oC = new OrigamiConverterHSB();
